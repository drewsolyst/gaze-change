...
int refractoryDuration = 700; # Length of timeout during which fixation on averted gaze eyes does not trigger direct gaze (ms)
int eyesX = 0; # x-position of the window around the eyes (pixels)
int eyesY = 0; # y-position of the window around the eyes (pixels)
int eyesW = 80; # half-width of window around the eyes (pixels)
int eyesH = 40; # half-height of window around the eyes (pixels)
...
int fixspotY = 250; # y-postition of the fixspot (pixels)
...
int trialDuration = 10000; # duration of trial (ms)
int imagewin = 0; #this is the extra room that the monkey has around the edge of the image (pixels)
					# so, if image win is 40, then there is a leniency of 10 pixels on all sides of the fixspot
int imagewidth = 800; #the width that all images should be standardized to (pixels)
int imageheight = 600; #the height that all images should be standardized to (pixels)
			...
			#~~~~~~~~~~~~~~~~ IF THE MONKEY MAINTAINED HIS FIXATION FOR THE REQUIRED AMOUNT OF TIME THEN GIVE JUICE AND BEGIN THE ITI ~~~~~~~~
				...
				begin
					...
					if iscan_x < eyesW && iscan_x > -eyesW && iscan_y < eyesH && iscan_y > -eyesH then
						timeInEye = (clock.time() - trialStart) - timeOutEye; # time spent viewing eye region
					else
				...
				then
					encode(18); #encode averted gaze image off;
					iti_real.present(); #present the iti to the monkey
					encode(04);
					iti_copy.present(); #present the iti to the experimenter
					iti_copy.set_part_x( 1, iscan_x ); # set the X eye position to eyeposition of the picture
